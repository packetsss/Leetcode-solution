...
#         self.x = 0
#         self.y = 0
#         self.node = grid[self.x][self.y]
#
#     def move_right(self):
#         self.y += 1
#         self.node = self.grid[self.x][self.y]
#
#     def move_down(self):
#         self.x += 1
#         self.node = self.grid[self.x][self.y]
#
#     def peak_right(self):
#         return self.grid[self.x][self.y + 1] == 1


def calculateM(dungeon):
    height, width = len(dungeon), len(dungeon[0])
    table = [[0 for _ in range(width)] for _ in range(height)]
    memo = {}  # remember common path
    total = set()
    last_move = 0  # right is 1, down is 2
    last_cord = (0, 0)

    temp = 0

    while table[0][0] != 1:  # moving all paths
        x, y = 0, 0
        temp += 1
        tot_sum, cur_sum, cumulate_sum = dungeon[0][0], dungeon[0][0], dungeon[0][0]
        add1 = False
        print(f"\npath: {temp}")

        while x < height - 1 or y < width - 1:  # moving 1 path

            if y == width - 1:  # hit right-edge
                if not add1:  # set barrier
                    table[x][y] = 1
                    add1 = True
                if x < height - 1:
                    x += 1  # move down

            elif x == height - 1:  # hit bottom-edge
                if not add1:  # set barrier
            ...
            elif table[x][y + 1] == 1:  # check right
                x += 1  # move down
            else:
                y += 1  # move right

            cumulate_sum += dungeon[x][y]
            tot_sum = min(cumulate_sum, tot_sum)
            print(np.array(table), f"\nposition: {(x, y)} sum: {tot_sum}")
...
def calc_m(dun):
    height, width = len(dun), len(dun[0])
    table = [[float("Inf") for _ in range(width + 1)] for _ in range(height + 1)]

    if width > 1:
        table[height - 1][width] = 1
    if height > 1:
        table[height][width - 1] = 1
    if width == height == 1:
        return -dun[0][0] + 1 if dun[0][0] < 0 else 1
    for i in reversed(range(height)):
        for j in reversed(range(width)):
            need = min(table[i + 1][j], table[i][j + 1]) - dun[i][j]
            table[i][j] = max(1, need)
    return table[0][0]
